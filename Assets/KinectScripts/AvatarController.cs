...
{	
	...
	protected int moveRate = 5;//valor 1
...
//		}
	...
	protected void MoveAvatar(uint UserID)
	{
		...
		Vector3 targetPos = Kinect2AvatarPos(trans, verticalMovement);//esta es la posicion a la que se debe trasladar, intenar aumentar el movimieto de esta cosa, es decir mandarlo mas lejos

		if(smoothFactor != 0f)
			bodyRoot.localPosition = Vector3.Lerp(bodyRoot.localPosition, targetPos, smoothFactor * Time.deltaTime);//quiza no suavizandolo tanto,solo aqui, aun que no esta funcionando * 500f
		else
